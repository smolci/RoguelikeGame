...
// Functions
void PauseMenu::addButton(const std::string key, const std::string text, unsigned int pos)
{
	float width = 250.f;
	float height = 50.f;
	float x = this->container.getPosition().x + this->container.getSize().x / 2.f - width / 2.f;
	float y = this->container.getPosition().y + this->container.getSize().y / 4 * pos;
...
void PauseMenu::update(const sf::Vector2i& mousePosWindow, sf::RenderWindow& window)
{
